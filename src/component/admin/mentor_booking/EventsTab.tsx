...
const DEFAULT_CSV_FIELDS: CSVExportField[] = [
    ...
    { key: 'reviewed_by', label: 'Người duyệt', enabled: false, category: 'registration' },
    { key: 'reviewed_at', label: 'Ngày duyệt', enabled: false, category: 'registration' },

    // Event Information
    { key: 'event_id', label: 'Event ID', enabled: false, category: 'event' },
    { key: 'event_title', label: 'Tên sự kiện', enabled: true, category: 'event' },
    { key: 'event_date', label: 'Ngày sự kiện', enabled: true, category: 'event' },
    { key: 'end_date', label: 'Ngày kết thúc', enabled: false, category: 'event' },
    { key: 'event_location', label: 'Địa điểm', enabled: true, category: 'event' },
    { key: 'event_type', label: 'Hình thức', enabled: true, category: 'event' },
    { key: 'max_participants', label: 'Số người tối đa', enabled: false, category: 'event' },
    ...
    { key: 'attendance_notes', label: 'Ghi chú check-in', enabled: false, category: 'attendance' },
...
export const EventsTab: React.FC<EventsTabProps> = ({
    ...
    // Get registration value for CSV
    const getRegistrationValue = (registration: EventRegistration, key: string): string => {
        ...
        switch (key) {
            ...
            case 'reviewed_by': return registration.reviewed_by || '';
            case 'reviewed_at':
                return registration.reviewed_at ? new Date(registration.reviewed_at).toLocaleString('vi-VN') : '';

            case 'event_id': return registration.event_id;
            case 'event_title': return event?.title || '';
            case 'event_date':
                return event?.event_date ? new Date(event.event_date).toLocaleString('vi-VN') : '';
            case 'end_date':
                return event?.end_date ? new Date(event.end_date).toLocaleString('vi-VN') : '';
            case 'event_location': return event?.location || '';
            case 'event_type': return event?.event_type || '';
            case 'max_participants': return event?.max_participants?.toString() || '';
            ...
            case 'attendance_notes': return attendance?.notes || '';
    ...
    const exportToCSV = async () => {
        try {
            ...
            onShowNotification('success', `Đã xuất ${dataToExport.length} đăng ký`);
    ...
    const updateRegistrationStatus = async (registrationId: string, status: string, adminNotes?: string) => {
        try {
            setSubmitting(true);
            const { data, error } = await supabase.rpc('events_admin_update_status', {
    ...
    const deleteRegistration = async (registrationId: string) => {
        if (!confirm('Bạn có chắc chắn muốn xóa đăng ký này? Thao tác này không thể hoàn tác.')) return;
        try {
            setSubmitting(true);
    ...
    const editReview = (review: any, registration: EventRegistration) => {
            ...
            <div className="p-6 bg-white rounded-xl shadow-sm mb-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-4">
                    ...
                    <select
                        ...
                        onChange={(e) => onFiltersChange({ eventTypeFilter: e.target.value })}
                        className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                    >
                ...
                <div className="flex justify-end gap-2">
                    ...
                    <button
                        ...
                        disabled={registrations.length === 0 && pagination.totalCount === 0}
                                        ...
                                        {registration.event_attendances && registration.event_attendances.length > 0 && (
                                            <div className="mt-3 p-3 bg-purple-50 rounded-lg border border-purple-200">
                                                ...
                                                <div className="text-sm text-gray-700 space-y-1">
                                                    ...
                                                    <div>
                                                        <strong>Phương thức:</strong> {registration.event_attendances[0].check_in_method === 'pass_code' ? 'Mã check-in' : registration.event_attendances[0].check_in_method === 'manual' ? 'Thủ công' : 'QR Code'}
                                                    </div>
                                                    {registration.event_attendances[0].notes && (
                                                        <div>
                                                            <strong>Ghi chú:</strong> {registration.event_attendances[0].notes}
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                        ...
                                        {registration.event_reviews && registration.event_reviews.length > 0 && (
                                            <div className="mt-3 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                                                ...
                                                <div className="flex gap-2">
                                                    <button
                                                        onClick={() => editReview(registration.event_reviews![0], registration)}
                    ...
                    <div className="relative bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex items-start">
                                    ...
                                    <label htmlFor="applyFilters" className="ml-2">
                                        ...
                                        <p className="text-xs text-gray-500">
                                            Xuất chỉ các đăng ký phù hợp với bộ lọc đang áp dụng
                                        </p>
                                    </label>
                                </div>
                                ...
                                <div className="flex items-start">
                                    ...
                                    <label htmlFor="exportAll" className="ml-2">
                                        ...
                                        <p className="text-xs text-gray-500">
                                            Xuất tất cả đăng ký (không chỉ trang hiện tại)
                                        </p>
                                    </label>
                                </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="border border-gray-200 rounded-lg p-4">
                                    <div className="flex items-center justify-between mb-3">
                                        <h5 className="font-medium text-gray-800">🎉 Thông tin Sự kiện</h5>
                                        <div className="flex gap-2">
                                            <button
                                                onClick={() => selectFieldsByCategory('event')}
                                                className="text-xs text-blue-600 hover:text-blue-800"
                                            >
                                                Chọn tất cả
                                            </button>
                                            <span className="text-gray-300">|</span>
                                            <button
                                                onClick={() => deselectFieldsByCategory('event')}
                                                className="text-xs text-blue-600 hover:text-blue-800"
                                            >
                                                Bỏ chọn
                                            </button>
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        {csvFields.filter(f => f.category === 'event').map((field) => (
                                            <div key={field.key} className="flex items-center">
                                                <input
                                                    type="checkbox"
                                                    id={`field-${field.key}`}
                                ...
                                <div className="text-sm text-gray-600 bg-gray-50 p-3 rounded">
                                    ...
                                    <div className="mt-1 text-xs">
                                        ...
                                        <span className="mr-3">🎉 Sự kiện: {csvFields.filter(f => f.category === 'event' && f.enabled).length}</span>
                            ...
                            <div className="bg-blue-50 p-4 rounded-lg">
                                <ul className="text-sm text-blue-800 space-y-1">
                                    <li>
                                        • Sẽ xuất: <strong>
                                        {csvOptions.exportAll
                                            ? `${Math.min(csvOptions.limit || 100, pagination.totalCount)} đăng ký`
                                            : `${registrations.length} đăng ký (trang hiện tại)`}
                                    </strong>
                                    </li>
                                    <li>
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div className="bg-gray-50 p-4 rounded-lg">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                    ...
                                    <div>
                                        <strong>Sự kiện:</strong> {editingRegistration.events?.title}
                                    </div>
                                    <div>
                                        <strong>Hình thức:</strong> <span className="capitalize">{editingRegistration.events?.event_type}</span>
                                    </div>
                                    <div>
                                        <strong>Ngày sự kiện:</strong> {editingRegistration.events?.event_date ? new Date(editingRegistration.events.event_date).toLocaleDateString('vi-VN') : ''}
                                    </div>
                                    <div>
                    ...
                    <div className="relative bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div className="bg-gray-50 p-4 rounded-lg">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                    <div>
                                        <strong>Người đánh giá:</strong> {editingRegistration?.profiles?.full_name}
                                    </div>
                                    <div>
                                        <strong>Sự kiện:</strong> {editingRegistration?.events?.title}
                                    </div>
